...
- tipo: "pago" SOMENTE se for comprovante de um pagamento JÁ EFETUADO. "boleto" se for boleto a pagar, cobrança, fatura ou guia de tributo (DARF/GPS/DAS) em aberto (pagamento solicitado, ainda não concluído). "nf" se for nota fiscal eletrônica (NF-e/DANFE) ainda não paga. "outro" se não for nenhum documento financeiro desses três tipos.
- valor: o valor da transação, do boleto/guia, ou o valor total da nota fiscal (número, ponto decimal).
- beneficiario: o nome de quem RECEBE o pagamento. Num boleto normal é o favorecido/beneficiário/cedente. Numa nota fiscal é o EMITENTE (quem vendeu/prestou o serviço — é ele que vai receber). Numa DARF, GPS ou outra guia de tributo federal/estadual (documento de arrecadação, sem cedente nomeado) o beneficiário é sempre o nome do órgão arrecadador (ex: "Receita Federal", "INSS") — NUNCA o nome/razão social do contribuinte que está pagando, mesmo que seja o único nome próprio visível no documento.
...
- Se não achar um campo, use null. Na dúvida entre pago e boleto, use "boleto". Na dúvida entre boleto e nf, use "nf" se tiver "DANFE"/"NF-e"/chave de acesso. Na dúvida se é um documento financeiro, use "outro".`;

export interface Extracao {
  /** true somente quando tipo === "pago" (mantido por compatibilidade com o restante do código). */
  pago: boolean;
  tipo: "pago" | "boleto" | "nf" | "outro";
  valor: number | null;
...
function parse(raw: string): {
  ...
  try {
    ...
    const tipo: "pago" | "boleto" | "nf" | "outro" =
      obj.tipo === "pago" || obj.tipo === "boleto" || obj.tipo === "nf" || obj.tipo === "outro" ? obj.tipo : "outro";
    return {
      pago: tipo === "pago",
      tipo,
